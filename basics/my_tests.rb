...
class WebPageTest < Minitest::Test
  ...
  def test_authors_statistics
    @page.instance_variable_set(:@articles, [@article1, @article2, @article3])
    assert_equal ({ 'author' => 2, 'author3' => 1 }), @page.authors_statistics
  end

  def test_best_author
    @page.instance_variable_set(:@articles, [@article1, @article2, @article3])
    assert_equal 'author', @page.best_author
  end

  def test_search
    article4 = Article.new('title4', 'body')
    @page.instance_variable_set(:@articles, [@article1, @article2, @article3, article4])
    assert_equal [ article4 ], @page.search('bo')
  end
end
